...
| 33  | [מהו Promise ?](#ש-מהו-promise-)|
| 34  | [למה אנחנו צריכים Promise ?](#ש-למה-אנחנו-צריכים-promise-)|
| 35  | [מהם שלושת המצבים של Promise ?](#ש-מהם-שלושת-המצבים-של-promise-)|
| 36  | [מהי פונקציית Callback ?](#ש-מהי-פונקציית-callback-)|
| 37  | [למה אנחנו צריכים פונקציות Callback ?](#ש-למה-אנחנו-צריכים-פונקציות-callback-)|
| 38  | [מהו Callback Hell ?](#ש-מהו-callback-hell-)|
| 39  | [מהו שרשור Promise ? (Promise Chaining)](#ש-מהו-שרשור-promise--promise-chaining)|
| 40  | [מה זה promise.all ?](#ש-מה-זה-promiseall-)|
| 41  | [מה המטרה של מטודת Race בPromise ?](#ש-מה-המטרה-של-מטודת-race-בpromise-)|
| 42  | [מה היתרונות והחסרונות של Promises בהשוואה לCallbacks ?](#ש-מה-היתרונות-והחסרונות-של-promises-בהשוואה-לcallbacks-)|
| 43  | [למה משמש סימן קריאה כפול ?](#ש-למה-משמש-סימן-קריאה-כפול-)|
| 44  | [למה משמש אופרטור המחיקה? ( delete operator )](#ש-למה-משמש-אופרטור-המחיקה--delete-operator-)|
| 45  | [למה משמש האופרטור typeof ?](#ש-למה-משמש-האופרטור-typeof-)|
| 46  | [מהי התכונה undefined ?](#שמהי-התכונה-undefined-)|
| 47  | [מהו הערך null ?](#ש-מהו-הערך-null-)|
| 48  | [מה ההבדל בין null וundefined ?](#ש-מה-ההבדל-בין-null-וundefined-)|
| 49  | [מה זו הפונקציה eval() ?](#ש-מה-זו-הפונקציה-eval-)|
| 50  | [איך ניגשים להיסטוריה בג'אווה סקריפט ?](#ש-איך-ניגשים-להיסטוריה-בגאווה-סקריפט-)|
...
## ש. מה זו הפונקציה eval() ?

ת. הפונקציה eval() אומדת קוד ג'אווה סקריפט שמיוצג כמחרוזת . המחרוזת יכולה להיות ביטוי , משתנה, הצהרה או סדרת הצהרות , נראה דוגמה :
...
## ש. איך ניגשים להיסטוריה בג'אווה סקריפט ?


ת. האובייקט window.history מכיל את ההיסטוריה של הדפדפן . אפשר לטעון את הכתובת הקודמת והכתובת הבא בשימוש במטודות back() וnext()
...
```
הערה : זה עובד גם בלי השימוש בתחילית window .
